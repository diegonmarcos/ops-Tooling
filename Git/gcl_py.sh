#!/bin/sh
# gcl_py.sh - Unified launcher for gcl.py
# Usage:
#   ./gcl_py.sh [command]           # Run natively (default)
#   ./gcl_py.sh --docker [command]  # Use Docker mode
#   ./gcl_py.sh --venv [command]    # Use venv mode
#   ./gcl_py.sh --help              # Show help
...
# Show help
show_help() {
    printf "${C_BOLD}${C_CYAN}gcl_py.sh - Git Clone/Pull/Push Manager Launcher${C_RESET}\n\n"
    printf "${C_BOLD}USAGE:${C_RESET}\n"
    printf "  ./gcl_py.sh [OPTIONS] [COMMAND] [ARGS...]\n\n"
    ...
    printf "  ./gcl_py.sh                    ${C_CYAN}# Run TUI natively${C_RESET}\n"
    printf "  ./gcl_py.sh status             ${C_CYAN}# Check status (native)${C_RESET}\n"
    printf "  ./gcl_py.sh --docker           ${C_CYAN}# Run TUI in Docker${C_RESET}\n"
    printf "  ./gcl_py.sh --docker status    ${C_CYAN}# Check status in Docker${C_RESET}\n"
    printf "  ./gcl_py.sh --venv sync        ${C_CYAN}# Sync using venv (auto-setup)${C_RESET}\n"
}
...
run_native() {
    ...
    if [ $# -eq 0 ]; then
        ...
        if ! command -v tput > /dev/null 2>&1; then
            ...
            if [ "$TERM_ROWS" -lt 24 ] || [ "$TERM_COLS" -lt 80 ]; then
                ...
                printf "Try: ./gcl_py.sh status  (to run CLI mode instead)\n" >&2
                exit 1
            fi
        fi
    ...
    python3 "$SCRIPT_DIR/gcl_py/gcl.py" "$@"
}

# Run via Docker
run_docker() {
    cd "$SCRIPT_DIR/gcl_py/gcl_py-docker"
...
# Run via venv
run_venv() {
    VENV_DIR="$SCRIPT_DIR/gcl_py/gcl_py-venv/venv"
    VENV_SCRIPT="$SCRIPT_DIR/gcl_py/gcl_py-venv/gcl-venv.sh"
